...
def resource_allocator(capacity, min, region):
    resource_count = 0
    while int(capacity) >= int(resource_capacity[min]):
        capacity = int(capacity) - int(resource_capacity[min])
        resource_count += 1
    return capacity, resource_count, resource_count * region[min]
...
def resource_cost(region, capacity, hours):
    ...
    while capacity != 0:
        min = keys[counter]
        (capacity,resource_count,cost) = resource_allocator(capacity, min,region)
        counter += 1
        if resource_count>0:
            resource.append((min,resource_count))
        total_cost += cost
